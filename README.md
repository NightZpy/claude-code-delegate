...
| `deepseek` | DeepSeek V4-Flash (`deepseek/deepseek-v4-flash`) | Fast debugging, diff review, boilerplate | 1M | $0.09 / $0.18 | 2 | ~Haiku 4.5 / GPT-5.4-mini | Cheapest of the fleet; V3.2-Exp has the one independently verified aider score (74.5%, #5) |
...
**Benchmark caveat:** most of the vendor-reported SWE-bench figures above 75% for these models are self-reported and not confirmed on neutral leaderboards. The only independently verified numbers here are GLM-5.2's SWE-bench Pro result (62.1%, morphllm-verified) and DeepSeek V3.2-Exp's aider result (74.5%).
