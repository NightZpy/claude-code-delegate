...
- **Trustworthy `--write` results:** an agentic `--write` job whose run changed NOTHING on disk now **fails** with `NO-OP WRITE` (the model returned code in its message instead of applying it) — never treat that as success. A completed write reports `applied (real diff) — files changed: …` grounded in the actual git diff, and flags `⚠ CLAIMED-BUT-NOT-APPLIED` when the prose claims a file it didn't change. Trust those lines, not the model's narrative; the real patch is on the result as `appliedPatch`.
