---
description: Delegate to Kimi — long-context audit and deep reasoning
argument-hint: '[--provider openrouter|siliconflow|deepinfra|cerebras] [--background] <brief>'
allowed-tools: Agent
---

Invoke the `frontier:frontier-runner` subagent via the `Agent` tool (`subagent_type: "frontier:frontier-runner"`), forwarding `--model kimi` followed by the raw user request as the prompt.
