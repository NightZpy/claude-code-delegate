...
### Fixed
- **Jobs dispatched by Claude Code were invisible from your own terminal.** Job state keyed off `CLAUDE_PLUGIN_DATA`, which is only set inside Claude Code (and points at whichever plugin's data dir — cc-delegate's state was landing under the *codex* plugin's folder). Run `cc-delegate jobs` / `status` / `watch` from a normal shell and the runtime looked somewhere else entirely, reporting "no jobs yet". State now lives at a deterministic path — `~/.claude/cc-delegate/state/` — identical in both contexts. Existing workspaces are migrated automatically on first use, so history and in-flight jobs are preserved.
